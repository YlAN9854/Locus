/**
 * 记录端 —— 拍照 + 打字/录音 + 自动定位 + AI 转录,一气呵成。
 *
 * 逻辑已下沉到 custom hook:
 *   usePhotoCapture   —— 拍照 + 落盘 + 定位
 *   useAudioRecording —— 录音 + AI 转录(不存 DB)
 * 本组件负责: 编排、打字保存、统一提交(save)、渲染。
 */
...
export default function RecordScreen() {
  ...
  const {draftRef, photoUri, poiLine, locating, busy: captureBusy, capture, resetCapture} = usePhotoCapture();
  ...
  const shutterBusy = captureBusy || saving;
  ...
  const save = async () => {
    ...
    try {
      ...
      clearTimeout(feedbackTimerRef.current);
      feedbackTimerRef.current = setTimeout(reset, 1500);
    } catch (e) {
